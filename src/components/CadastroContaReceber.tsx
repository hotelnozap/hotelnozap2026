...
export interface CadastroContaReceberProps {
  onBack: () => void;
  onSaveSuccess?: () => void;
}

export const CadastroContaReceber: React.FC<CadastroContaReceberProps> = ({ onBack, onSaveSuccess }) => {
  // Form states
  ...
  // Modal State for Nova Condição de Recebimento
  ...
  const [accountDestination, setAccountDestination] = useState('caixa_recepcao');
  
  // List of accounts state
  const [accountOptions, setAccountOptions] = useState([
    { value: 'caixa_recepcao', label: 'Gaveta / Caixa Recepção' },
    { value: 'banco_itau', label: 'Banco Itaú - Conta Corrente' },
    { value: 'banco_bb', label: 'Banco do Brasil' },
    { value: 'cofre', label: 'Cofre Central' },
  ]);

  // Modal State for Nova Conta de Destino / Depósito
  ...
  const handleSaveNovaConta = (e: React.FormEvent) => {
    ...
    setAccountDestination(newValue);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!description.trim()) {
      showToast('Por favor, informe a descrição do recebível.');
      return;
    }
    if (!category) {
    ...
    if (!customer.trim()) {
      showToast('Por favor, informe o cliente/hóspede/empresa.');
      return;
    }
    ...
    <div className="bg-[#f8f9fc] min-h-screen p-4 sm:p-6 md:p-10 text-slate-800 font-sans antialiased pb-28 sm:pb-16">
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 sm:pb-6 border-b border-slate-200 gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
              Preencha as informações do recebível, cliente/hóspede, vencimento e conciliação.
            </p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/80 shadow-xs space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-slate-100 gap-3">
              ...
              <div className="flex items-center justify-between sm:justify-end gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-200/60">
                <span className="text-xs font-medium text-slate-500 uppercase tracking-wider">Situação do Recebível</span>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs sm:text-sm font-semibold ${statusPendente ? 'text-amber-700' : 'text-emerald-700'}`}>
                    {statusPendente ? 'Pendente' : 'Recebida'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
              <div className="md:col-span-2">
                <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wide">
                  Descrição do Recebível / Título <span className="text-rose-500">*</span>
                </label>
                <input 
                  type="text" 
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Ex: Diárias Hóspede - Reserva #HK-8891 / Faturamento Empresa Conveniada" 
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wide">
                  Categoria de Recebimento <span className="text-rose-500">*</span>
                </label>
                <select 
                  value={category}
                ...
                >
                  ...
                  <option value="frigobar">Consumo Frigobar & Restaurante</option>
                  <option value="eventos">Locação de Salas & Eventos</option>
                  <option value="faturamento">Faturamento Empresa / Convênio</option>
                  <option value="servicos">Serviços Extras & Passeios</option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wide">
                  Cliente / Hóspede / Empresa <span className="text-rose-500">*</span>
                </label>
                <div className="relative">
                  <input 
                    ...
                    placeholder="Ex: Roberto Silveira / TechCorp LTDA" 
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wide">
                  Nº da Reserva / Documento / Fatura
                </label>
                <input 
                  type="text" 
                  value={documentNumber}
                  onChange={(e) => setDocumentNumber(e.target.value)}
                  placeholder="Ex: RES-2026-892 / FAT-8492" 
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003400] text-sm bg-slate-50/50 focus:bg-white transition-all"
                />
              </div>
            </div>
          ...
          {/* BLOCO 2: VALORES, VENCIMENTO & RECEBIMENTO */}
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/80 shadow-xs space-y-5">
            <div className="flex items-center gap-2.5 pb-4 border-b border-slate-100">
              <span className="p-2 rounded-lg bg-emerald-50 text-[#003400] shrink-0">
                <span className="material-symbols-outlined text-xl">event_available</span>
              </span>
              <span className="font-semibold text-slate-900 text-sm sm:text-base">Valores, Vencimento & Recebimento</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5 uppercase tracking-wide">
                  Valor Total a Receber (R$) <span className="text-rose-500">*</span>
                </label>
                <div className="relative">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003400] text-xs sm:text-sm font-semibold text-emerald-800 bg-slate-50/50 focus:bg-white transition-all"
                  required
                />
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <option value="transferencia">Transferência TED/PIX Empresarial</option>
                  <option value="dinheiro">Dinheiro (Caixa Recepção)</option>
                </select>
              </div>
            </div>

            {/* PARCELAMENTO E DESTINO */}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    title="Cadastrar Nova Condição de Recebimento"
                  >
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="w-10 h-10 rounded-xl bg-[#d1fae5] hover:bg-emerald-200 border border-emerald-400 text-black font-bold flex items-center justify-center shrink-0 shadow-xs transition-all cursor-pointer"
                    title="Cadastrar Nova Conta de Destino / Depósito"
                  >
                    <span className="material-symbols-outlined text-lg text-black font-bold">add</span>
                  </button>
                </div>
          ...
          {/* BLOCO 3: ANEXO DE COMPROVANTE / VOUCHER */}
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/80 shadow-xs space-y-4">
            <div className="flex items-center gap-2.5 pb-3 border-b border-slate-100">
              <span className="p-2 rounded-lg bg-emerald-50 text-[#003400] shrink-0">
                <span className="material-symbols-outlined text-xl">attach_file</span>
              </span>
              <span className="font-semibold text-slate-900 text-sm sm:text-base">Anexo do Documento (Comprovante, Vouchers ou Fatura)</span>
            </div>
            ...
            >
              ...
              <p className="text-[10px] sm:text-xs text-slate-400 mt-1">Formatos aceitos: PDF, JPG, PNG até 10MB (Comprovante, Voucher, NF-e)</p>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div>
              ...
              <textarea 
                ...
                placeholder="Instruções de cobrança, chave PIX para recebimento, dados do contrato de faturamento ou notas do turno..." 
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-[#003400] text-xs sm:text-sm bg-slate-50/50 focus:bg-white transition-all"
              />
            </div>
          </div>
      ...
      {/* MODAL DE CADASTRO DE NOVA CONTA DE DESTINO / DEPÓSITO */}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-[#003400] text-white px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-bold text-base text-white">Nova Conta de Destino / Depósito</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveNovaConta} className="p-5 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="cofre">Cofre Central</option>
                </select>
              </div>
      ...
      {/* MODAL DE CADASTRO DE NOVA CONDIÇÃO DE RECEBIMENTO */}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150">
            ...
            <div className="bg-[#003400] text-white px-5 py-4 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <h3 className="font-bold text-base text-white">Nova Condição de Recebimento</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSaveNovaCondicao} className="p-5 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="Ex: Faturamento 30 Dias, Entrada + 2x, 15/30 Dias..."
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Instruções adicionais da condição de recebimento..."
...
export default CadastroContaReceber;
