...
export function mapPlanoDBToFrontend(row: any): any {
  ...
  return {
    ...
    roomLimit: Number(row.limite_quartos) || 15,
    roomLimitText: `Capacidade para até ${row.limite_quartos || 15} quartos`,
    ...
    whatsappConnections: Number(row.conexoes_whatsapp) || 1,
    whatsappConnectionsText: `${row.conexoes_whatsapp || 1} Conexão${Number(row.conexoes_whatsapp || 1) > 1 ? 'ões' : ''} WhatsApp simultâneas`,
